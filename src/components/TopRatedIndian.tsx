...
const TopRatedIndian: React.FC<Props> = ({ searchQuery }) => {
  ...
  useEffect(() => {
    const fetchMovies = async () => {
      try {
        const data = await getMovies("top-rated-india"); // Call the API to fetch movies in theaters
        setMovies(data || []);
        setStatus("LOADED");
      } catch (error) {
        setError(
