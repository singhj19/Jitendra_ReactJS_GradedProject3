...
const MovieItem: React.FC<Props> = ({ movie, filter, favourites, updateFavourites }) => {
    ...
    return (
        <Card className="card-custom">
            <Link to={`/${filter}/${id}`}>
                <Card.Img variant="top" src={posterurl} className="card-img-custom" />
            </Link>
            <Card.Body className="card-body-custom">
                <Card.Title className="card-title-custom">{title}</Card.Title>
                <Card.Text className="card-text-custom">
                    <button onClick={handleFavouriteClick} className="btn btn-link btn-favourite" disabled={isUpdating}>
                        {
                            filter === 'favourite'
                                ? (
                                    <>
