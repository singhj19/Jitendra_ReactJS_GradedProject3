...
const MoviesInTheater: React.FC<Props> = ({ searchQuery }) => {
  ...
  useEffect(() => {
    const fetchMovies = async () => {
      try {
        const data = await getMovies("movies-in-theaters"); // Call the API to fetch movies in theaters
        setMovies(data || []);
        setStatus("LOADED");
      } catch (error) {
        setError(
